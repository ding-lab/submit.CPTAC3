...
# NOTE: this will have to be substantially rewritten to deal cleanly with various scenarios such as
#  * RNA-Seq with Tumor and Adjacent Normal, the latter existing much of the time but not always
#  * Adjacent-normal somatic analysis
#  * "WGS CNV" analysis, which analyzes tumor and normal individually
#  * Headers get complicated with the various scenarios
# Ideally, we wouldn't have to guess here about what the input files are, but these would be passed
# as inputs (and ideally generated by the pipeline)
...
#  * RNA-Seq-TA: two FASTQ data files (R1, R2) associated with each case ("RNA-Seq T"), and two with "RNA-Seq A"
#  * adjacent_somatic: a paired tumor and tissue adjacent sample associated with each case ("W?S T+A")
#  * adjacent_cnv: tumor and tissue adjacent samples are analyzed individually for each case ("W?S T,A")
# Development notes: Ideally, upstream analyses should report explicitly what files they used, rather than us
# having to assume it here.  
#
...
TUMOR_SUFFIX=$3  # T or N if MANIFEST_TYPE=cnv, T or A if adjacent_cnv, ignored otherwise
...
elif [[ $MANIFEST_TYPE == "cnv" || $MANIFEST_TYPE == "adjacent_cnv" ]]; then
...
elif [[ $MANIFEST_TYPE == "adjacent_somatic" ]]; then
    HEADER="cancer\tcase_ID\tinput_experimental_strategy\ttumor_submitter_id\ttumor_UUID\ttissue_adjacent_submitter_id\ttissue_adjacent_UUID\treference_version\tfile_type\tfile_name\tfile_size\tmd5sum\n" 
else
...
while read CASE; do
    ...
    if [ -z $DRY_RUN ]; then 
        ...
        elif [ $MANIFEST_TYPE == "adjacent_cnv" ]; then
            process_case $CASE $COI T >> $MANIFEST_FN
            process_case $CASE $COI A >> $MANIFEST_FN
        else 
